/*
 * Copyright 2006-2019 (c) Care.com, Inc.
 * 1400 Main Street, Waltham, MA, 02451, U.S.A.
 * All rights reserved.
 *
 * This software is the confidential and proprietary information of
 * Care.com, Inc. ("Confidential Information").  You shall not disclose
 * such Confidential Information and shall use it only in accordance with
 * the terms of an agreement between you and CZen.
 */
package com.orange.auction.event.member;

import com.orange.auction.event.AbstractEvent;
import com.orange.auction.model.Member;

/**
 * Created 10/12/2019
 *
 * @author sjkumar
 */
public class MemberProfileEditedEvent extends AbstractEvent {

    public static final String EVENT_NAME = "MEMBER_PROFILE_UPDATED_EVENT";
    private final Member updatedMember;

    public MemberProfileEditedEvent(Long memberId, Member updatedMember) {
        super(EVENT_NAME, memberId);
        this.updatedMember = updatedMember;
    }
}
